...
<script>
  ...
  async function resetJob(){try{await api('/ui/api/jobs/reset',{method:'POST'});clearTimeout(jobTimer);byId('dailyEnd').value='';byId('backfillSymbols').value='';byId('backfillStart').value='';byId('backfillEnd').value='';renderJob(null);showNotice('Formulir dan status scraping sudah direset. Data tersimpan tidak dihapus.','success')}catch(error){showNotice(error.message,'error')}}
  function resetView(){byId('symbols').value='';byId('fromDate').value='';byId('toDate').value='';byId('dataBody').innerHTML='<tr><td class="empty" colspan="8">Tampilan direset. Isi filter lalu klik Tampilkan.</td></tr>';setExportsEnabled(false);showNotice('Filter dan tabel data sudah dikosongkan.','success')}
